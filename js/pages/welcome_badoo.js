$(document).ready(function() {
 ...
 //     	 autoPlay: 3000, //Set AutoPlay to 3 seconds
	...
	$("header").css("height",w-half);

	$(".submit-btn").click(function(){
	    $(".initialblock").hide();
		$(".register-block").show();
		if($(".join-the-best-bating").innerHeight() > 500){
			$(".apple-google-window").css("padding-top",400);	
		}else{
			$(".apple-google-window").css("padding-top",100);
		}
	});
	...
	$(".close-btn").click(function(){
	    $(".close-btn").hide();
		$(".light-box").hide();
		$(".forgetpasslightbox").hide();
	});
	$(".signinclass").click(function(){
	    $(".close-btn").show();
		$(".light-box").show();
	});
	$(".forgetpassworclass").click(function(){
	    $(".close-btn").show();
		$(".forgetpasslightbox").show();
		$(".light-box").hide();
	});

	$(".fp-clsbtn").click(function(){
		$(".light-box").show();
		$(".close-btn").show();
	});
...
$(".sign-up-content").html("<div class='alert alert-success' style='text-align:center; font-size:18px;'><i class='fa fa-check-circle' style='font-size:40px;'></i><br />" + recover_password_success_str + "</div>");					
					setTimeout(function() {
						window.location = data.url_redirect;
					}, 1000);
				}
